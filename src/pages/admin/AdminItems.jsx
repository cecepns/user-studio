...
const AdminItems = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('https://api.kingcreativestudio.my.id/user-studio/api/items/categories');
      const data = await response.json();
      setCategories(data);
    } catch (error) {
  ...
  const handleItemSubmit = async (itemData) => {
    try {
      const url = itemData.id 
        ? `https://api.kingcreativestudio.my.id/user-studio/api/items/${itemData.id}`
        : 'https://api.kingcreativestudio.my.id/user-studio/api/items';
  ...
  const handleDeleteItem = async (id) => {
    ...
    try {
      const response = await fetch(`https://api.kingcreativestudio.my.id/user-studio/api/items/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('admin_token')}`
        }
